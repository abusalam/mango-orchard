<x-admin-layout title="Pragati Darpan access" active="monitoring-access">
    <header class="mb-6">
        <div class="flex items-end justify-between gap-4 flex-wrap">
            <div>
                ...
                <p class="mt-1 text-stone-600 dark:text-stone-300 text-sm">Grant the <code>Samikshak</code> role and create a profile in one click. Users without a profile here will never see the Pragati Darpan dashboard, even if they hold the role from elsewhere.</p>
            </div>
            <p class="text-sm text-stone-500 dark:text-stone-400" data-testid="monitoring-member-count">{{ $memberCount }} {{ $memberCount === 1 ? 'member' : 'members' }}</p>
        </div>
    ...
    >
            ...
            <a href="{{ route('admin.monitoring.access.index') }}" class="text-xs text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:text-stone-100 underline">Reset</a>
        </form>
    </header>

    <div class="bg-white dark:bg-stone-950 rounded-2xl border border-stone-200 dark:border-stone-800 overflow-hidden" data-testid="access-table">
        @if ($users->isEmpty())
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                    @foreach ($users as $user)
                        ...
                        <tr class="odd:bg-white dark:odd:bg-stone-950 dark:bg-stone-950 even:bg-stone-50/50 dark:even:bg-stone-900" data-testid="access-row-{{ $user->id }}">
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-xs text-stone-600 dark:text-stone-300 hidden sm:table-cell">
                                @forelse ($user->roles->where('name', '!=', \App\Roles::MONITOR)->where('name', '!=', \App\Roles::MONITOR_ADMIN) as $role)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 mr-1 mb-1">{{ $role->name }}</span>
                                @empty
                            ...
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 text-xs" data-testid="status-non-member">Not added</span>
                                @endif
                            </td>
    ...
    <div class="mt-8 text-sm text-stone-500 dark:text-stone-400">
        <p><strong>Next step after granting access:</strong> place the user in the org chart and tag their designations from <a href="{{ route('admin.monitoring.hierarchy.index') }}" class="underline hover:text-stone-900 dark:text-stone-100">Hierarchy</a>.</p>
    </div>
</x-admin-layout>
