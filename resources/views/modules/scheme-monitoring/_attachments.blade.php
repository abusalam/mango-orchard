...
<div data-testid="attachments-panel">
    <h2 class="text-lg font-semibold text-stone-900">Attachments</h2>

    @if ($attachable->attachments->isEmpty())
        <p class="mt-2 text-sm text-stone-500">No files attached yet.</p>
    @else
        <ul class="mt-3 divide-y divide-stone-100 border border-stone-200 rounded-xl overflow-hidden bg-white">
            @foreach ($attachable->attachments as $attachment)
                <li class="flex items-center gap-3 px-4 py-3" data-testid="attachment-row-{{ $attachment->id }}">
                    ...
                    <div class="min-w-0 flex-1">
                        <a href="{{ $attachment->url() }}" target="_blank" rel="noopener" class="font-medium text-stone-900 hover:underline break-words">
                    ...
                    <form method="POST" action="{{ route('monitoring.attachments.destroy', $attachment) }}" class="shrink-0">
                        @csrf @method('DELETE')
                        <button type="submit" class="text-xs text-rose-700 hover:underline" data-testid="attachment-delete-{{ $attachment->id }}">Remove</button>
                    </form>
                </li>
            @endforeach
        </ul>
    ...
    <form method="POST" action="{{ $uploadRoute }}" enctype="multipart/form-data" class="mt-4 flex flex-wrap items-end gap-3">
        ...
        <div class="flex-1 min-w-[14rem]">
            <label class="block text-sm font-medium text-stone-700">Add an attachment</label>
            <input
                type="file"
                name="file"
                required
                class="mt-1 block w-full text-sm text-stone-700 file:mr-3 file:px-3 file:py-1.5 file:rounded-full file:border-0 file:bg-stone-900 file:text-amber-50 file:text-xs file:font-medium hover:file:bg-stone-800"
                data-testid="attachment-file-input"
            >
            <p class="mt-1 text-xs text-stone-500">Up to 10 MB per file.</p>
