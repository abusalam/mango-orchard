                ...
                <div class="rounded-xl border border-stone-200 p-4 sm:p-5">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            @if ($role->permissions->isNotEmpty())
                                <p class="mt-1 text-xs text-stone-500">{{ __('Grants:') }} {{ $role->permissions->pluck('name')->implode(', ') }}</p>
                            @else
                                <p class="mt-1 text-xs text-stone-500">{{ __('No specific permissions attached — informational role.') }}</p>
                            @endif
                        </div>

                        @if ($pending)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium bg-amber-100 text-amber-900 border border-amber-200 whitespace-nowrap">{{ __('Pending review') }}</span>
                        @endif
                    </div>
                    ...
                    @if ($lastReviewed && ! $pending)
                        <p class="mt-3 text-xs text-stone-500">
                            ...
                            <span @class([
                                ...
                                'text-rose-700' => $lastReviewed->status === $roleApplicationStatuses['rejected'],
                    ...
                    @else
                        <form method="POST" action="{{ route('role-applications.store') }}" class="mt-4 space-y-3">
                            ...
                            <div>
                                <label for="message_{{ $role->id }}" class="block text-xs font-medium text-stone-700">{{ __('Why do you need this role?') }} <span class="text-stone-400 font-normal">({{ __('optional') }})</span></label>
                                <textarea id="message_{{ $role->id }}" name="message" rows="2" maxlength="1000"
                                          class="mt-1 block w-full rounded-lg border-stone-300 shadow-sm focus:border-orange-400 focus:ring-orange-400 text-sm"
