<section data-testid="profile-delegations">
    ...
    @else
        <div class="mt-6 space-y-4">
            @foreach ($delegatableRolesHeld as $role)
                <div class="rounded-xl border border-stone-200 p-4 sm:p-5">
                    <p class="text-sm font-semibold text-stone-900">{{ Str::headline($role->name) }} <span class="text-stone-400 font-normal">({{ $role->name }})</span></p>
                    <p class="mt-1 text-xs text-stone-500">{{ __('Delegate this role to another user by email. They\'ll get it immediately.') }}</p>
                    ...
                    <form method="POST" action="{{ route('role-delegations.store') }}" class="mt-3 flex flex-col sm:flex-row gap-2">
                        ...
                        <div class="flex-1">
                            <input type="email" name="recipient_email" required maxlength="255"
                                   ...
                                   class="block w-full rounded-lg border-stone-300 shadow-sm focus:border-orange-400 focus:ring-orange-400 text-sm"
    ...
    @if ($delegationsGranted->isNotEmpty())
        <div class="mt-8">
            <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500 mb-2">{{ __('You\'ve delegated to') }}</h3>
            <ul class="divide-y divide-stone-100 text-sm rounded-xl border border-stone-200 overflow-hidden bg-white" data-testid="delegations-granted">
                @foreach ($delegationsGranted as $delegation)
                    <li class="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div>
                            <p class="text-stone-800"><strong>{{ $delegation->recipient->name }}</strong>
                                <span class="text-stone-500">{{ $delegation->recipient->email }}</span></p>
                        ...
                        >
                            <button type="button" class="text-xs text-rose-700 hover:text-rose-900 font-medium" data-testid="revoke-delegation-granted">{{ __('Revoke') }}</button>
                        </x-confirm-form>
                    </li>
                @endforeach
            </ul>
    ...
    @if ($delegationsReceived->isNotEmpty())
        <div class="mt-8">
            <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500 mb-2">{{ __('Delegated to you') }}</h3>
            <ul class="divide-y divide-stone-100 text-sm rounded-xl border border-stone-200 overflow-hidden bg-white" data-testid="delegations-received">
                @foreach ($delegationsReceived as $delegation)
                    <li class="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div>
                            <p class="text-stone-800"><span class="font-mono">{{ $delegation->role?->name }}</span></p>
                            <p class="text-xs text-stone-500">
                                {{ __('Granted by') }} <strong class="text-stone-700">{{ $delegation->delegator?->name ?? __('a removed user') }}</strong>
                        ...
                        <x-confirm-form
                            ...
                            variant="warning"
                        >
                            <button type="button" class="text-xs text-rose-700 hover:text-rose-900 font-medium" data-testid="revoke-delegation-received">{{ __('Renounce') }}</button>
                        </x-confirm-form>
                    </li>
                @endforeach
            </ul>
