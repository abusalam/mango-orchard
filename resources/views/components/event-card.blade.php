...
<article class="group relative overflow-hidden rounded-2xl bg-white border border-stone-200/80 hover:border-stone-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
    <a href="{{ route('events.show', $event) }}" class="block">
        ...
        <div class="p-5">
            ...
            <p class="mt-1 text-sm text-stone-500">{{ $isOnline ? 'Online' : $event->location }}</p>
            <p class="mt-3 text-sm font-medium text-stone-800">{{ $event->start_at->format('D, M j') }} <span class="text-stone-500 font-normal">· {{ $event->start_at->format('g:i A') }}</span></p>
            @if ($event->host)
                <p class="text-xs text-stone-500">Hosted by {{ $event->host }}</p>
            @endif
        </div>
    </a>
</article>
