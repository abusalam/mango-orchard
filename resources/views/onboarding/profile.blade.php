<x-onboarding-layout title="Tell us about you" step="profile">
    <div class="rounded-2xl bg-white border border-stone-200 shadow-sm p-6 sm:p-10">
        ...
        <p class="mt-2 text-stone-600">A couple of quick questions so we can tailor your orchard.</p>

        <form method="POST" action="{{ route('onboarding.profile') }}" class="mt-8 space-y-6">
            @csrf

            <div>
                <label for="region" class="block text-sm font-medium text-stone-800">Where are you mango-watching from?</label>
            ...
            <fieldset>
                <legend class="block text-sm font-medium text-stone-800">How would you describe yourself?</legend>
                ...
                @error('expertise') <p class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
            </fieldset>
