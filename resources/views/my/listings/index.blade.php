<x-site-layout :title="'My listings — '.config('app.name')">
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
            <div>
                ...
                <p class="mt-2 text-stone-600 dark:text-stone-300">Mangoes you've put up for the marketplace. Drafts only show here; published listings appear at <a href="{{ route('listings.index') }}" class="text-orange-700 hover:text-orange-900">/listings</a>.</p>
            </div>
        ...
        @if ($listings->isEmpty())
            <div class="rounded-2xl border border-dashed border-stone-300 p-12 text-center">
                @can(\App\Permissions::LISTINGS_MANAGE)
                    <p class="text-stone-600 dark:text-stone-300">You haven't listed anything yet.</p>
                    <a href="{{ route('my.listings.create') }}" class="mt-4 inline-block text-orange-700 font-medium">List your first variety →</a>
                @else
                    <p class="text-stone-600 dark:text-stone-300">You don't have any listings.</p>
                    <p class="mt-2 text-sm text-stone-500 dark:text-stone-400">Listing requires the <code class="px-1.5 py-0.5 bg-stone-100 dark:bg-stone-800 rounded text-stone-700 dark:text-stone-300">grower</code> role — ask an admin to grant it.</p>
                @endcan
            </div>
        @else
                    ...
                    <article class="group relative overflow-hidden rounded-2xl bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-800 hover:shadow-lg transition-shadow">
                        ...
                        <div class="p-5">
                            ...
                            @if ($listing->price_per_kg)
                                <p class="mt-3 text-sm text-stone-800 dark:text-stone-200">₹{{ number_format((float) $listing->price_per_kg, 2) }} / kg</p>
                            @endif
                            <div class="mt-5 flex gap-2 text-xs">
                                <a href="{{ route('my.listings.edit', $listing) }}" class="px-2.5 py-1 rounded border border-stone-200 dark:border-stone-800 hover:border-stone-400 transition-colors">Edit</a>
                                @if ($listing->status !== \App\Modules\MangoOrchard\Models\Listing::STATUS_DRAFT)
                                    <a href="{{ route('listings.show', $listing) }}" target="_blank" rel="noopener" class="px-2.5 py-1 rounded border border-stone-200 dark:border-stone-800 hover:border-stone-400 transition-colors">View public →</a>
                                @endif
                                <x-confirm-form
