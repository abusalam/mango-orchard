<x-admin-layout title="Roles" active="roles">
    <header class="mb-6 flex items-end justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-stone-600 dark:text-stone-300">Group permissions into roles, then assign roles to users.</p>
        </div>
            ...
            <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                @foreach ($roles as $role)
                    ...
                    <tr class="odd:bg-stone-50/60 dark:odd:bg-stone-900 hover:bg-amber-50/60 dark:hover:bg-stone-800 transition-colors">
                        <td class="px-5 py-4 font-medium text-stone-900 dark:text-stone-100">
                            {{ $role->name }}
                            @if ($isSuper)
                        ...
                        <td class="px-5 py-4 text-right">
                            @unless ($isSuper)
                                <div class="flex justify-end gap-2 text-sm">
                                    ...
                                    >
                                        <button type="button" class="text-rose-700 dark:text-rose-400 hover:text-rose-900 font-medium">Delete</button>
                                    </x-confirm-form>
                                </div>
                            @endunless
                        </td>
