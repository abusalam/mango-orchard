<x-admin-layout title="Mango Orchard access" active="mango-access">
    <header class="mb-6">
        <div class="flex items-end justify-between gap-4 flex-wrap">
            <div>
                ...
                <p class="mt-1 text-stone-600 text-sm">Grant the <code>mango-orchard-member</code> role so the user can self-apply for grower, curator, convener or advisor on their profile page. You can pre-assign any of those sub-roles in the same step.</p>
            </div>
            <p class="text-sm text-stone-500" data-testid="mango-member-count">{{ $memberCount }} {{ $memberCount === 1 ? 'member' : 'members' }}</p>
        </div>
        ...
        <form method="GET" class="mt-4 flex flex-wrap gap-3 items-end">
            <div>
                ...
                <input type="text" name="q" value="{{ $search }}" placeholder="name or email"
                       class="rounded-lg border-stone-300 text-sm" data-testid="mango-access-search">
            </div>
            <div>
            ...
            <a href="{{ route('admin.mango-orchard.access.index') }}" class="text-xs text-stone-500 hover:text-stone-900 underline">Reset</a>
        </form>
    </header>

    <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden" data-testid="mango-access-table">
        @if ($users->isEmpty())
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-stone-100">
                    @foreach ($users as $user)
                        ...
                        <tr class="odd:bg-white even:bg-stone-50/50" data-testid="mango-access-row-{{ $user->id }}">
                            <form method="POST" action="{{ route('admin.mango-orchard.access.grant', $user) }}">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @if (! empty($currentSub))
                                        <p class="text-[10px] text-stone-500 mt-1">Holds: {{ implode(', ', $currentSub) }}</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-xs hidden md:table-cell">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($subRoles as $sr)
                                            <label class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-stone-200 bg-stone-50 cursor-pointer has-[:checked]:bg-amber-50 has-[:checked]:border-amber-300">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-stone-100 text-stone-600 text-xs" data-testid="mango-status-non-member">Not added</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                                    ...
                                    @else
                                        <button type="submit"
                                                class="inline-flex items-center px-3 py-1.5 rounded-full bg-stone-100 text-stone-700 hover:bg-stone-200 text-xs"
    ...
    <div class="mt-8 text-sm text-stone-500">
        <p><strong>Note:</strong> Once added, the user can self-apply for grower / curator / convener / advisor from their profile page. Admins can also assign sub-roles directly via <a href="{{ route('admin.users.index') }}" class="underline hover:text-stone-900">Users</a>.</p>
    </div>
</x-admin-layout>
