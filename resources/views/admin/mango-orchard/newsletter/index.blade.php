<x-admin-layout title="Newsletter" active="mango-newsletter">
    <header class="mb-6 flex items-end justify-between gap-4 flex-wrap">
        <div>
            ...
            <p class="mt-1 text-stone-600 dark:text-stone-300 text-sm">Compose and send monthly issues to opted-in subscribers.</p>
        </div>
        <div class="flex items-end gap-4">
            <p class="text-sm text-stone-500 dark:text-stone-400" data-testid="newsletter-subscriber-count">
                <span class="font-medium text-stone-900 dark:text-stone-100">{{ $subscriberCount }}</span> {{ Str::plural('subscriber', $subscriberCount) }}
            </p>
    ...
    <section class="mb-10" data-testid="newsletter-drafts">
        <h2 class="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-3">Drafts</h2>
        <div class="bg-white dark:bg-stone-950 rounded-2xl border border-stone-200 dark:border-stone-800">
            @if ($drafts->isEmpty())
                <p class="px-6 py-12 text-center text-stone-500 dark:text-stone-400 text-sm">No drafts in flight.</p>
            @else
                <ul class="divide-y divide-stone-100 dark:divide-stone-800">
                    @foreach ($drafts as $issue)
                        <li class="px-4 py-3 flex flex-wrap items-center justify-between gap-3" data-testid="newsletter-draft-{{ $issue->id }}">
                            <div class="min-w-0">
                                <p class="font-medium text-stone-900 dark:text-stone-100 truncate">{{ $issue->subject }}</p>
    ...
    <section data-testid="newsletter-sent">
        <h2 class="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-3">Sent</h2>
        <div class="bg-white dark:bg-stone-950 rounded-2xl border border-stone-200 dark:border-stone-800">
            @if ($sentIssues->isEmpty())
                <p class="px-6 py-12 text-center text-stone-500 dark:text-stone-400 text-sm">No issues sent yet.</p>
            @else
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                        @foreach ($sentIssues as $issue)
                            <tr class="odd:bg-white dark:odd:bg-stone-950 dark:bg-stone-950 even:bg-stone-50/50 dark:even:bg-stone-900" data-testid="newsletter-sent-{{ $issue->id }}">
                                <td class="px-4 py-3 font-medium text-stone-900 dark:text-stone-100">{{ $issue->subject }}</td>
                                <td class="px-4 py-3 text-xs text-stone-500 dark:text-stone-400 hidden sm:table-cell">{{ $issue->sent_at->format('d M Y H:i') }}</td>
                                <td class="px-4 py-3 text-stone-700 dark:text-stone-300">{{ $issue->sent_to_count }}</td>
                                <td class="px-4 py-3 text-xs text-stone-500 dark:text-stone-400 hidden md:table-cell">{{ $issue->author?->name ?? '—' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
