<x-admin-layout :title="'Edit '.$user->name" active="users">
    <header class="mb-6">
        <p class="text-sm text-stone-500"><a href="{{ route('admin.users.index') }}" class="hover:text-stone-900">Users</a> / Edit</p>
        <h1 class="mt-2 text-3xl font-semibold tracking-tight">{{ $user->name }}</h1>
        <p class="mt-1 text-stone-600">{{ $user->email }}</p>
    </header>

    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="bg-white rounded-2xl border border-stone-200 p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')

        <fieldset>
            <legend class="block text-sm font-medium text-stone-800">Roles</legend>
            <p class="mt-1 text-xs text-stone-500">Pick one or more roles. Permissions are inherited from each assigned role.</p>
            ...
            <div class="mt-4 space-y-2">
                @foreach ($roles as $role)
                    ...
                    <label class="flex items-start gap-3 p-3 rounded-xl border border-stone-200 cursor-pointer hover:border-orange-300 has-[:checked]:border-orange-500 has-[:checked]:bg-orange-50 transition-colors">
                        ...
                        <span>
                            <span class="block font-medium text-stone-900">{{ $role->name }}</span>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-stone-100">
            ...
            <a href="{{ route('admin.users.index') }}" class="text-sm text-stone-600 hover:text-stone-900">Cancel</a>
        </div>
    </form>
</x-admin-layout>
