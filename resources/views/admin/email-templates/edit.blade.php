<x-admin-layout :title="'Edit '.$template->name" active="email-templates">
    ...
    <header class="mb-6">
        ...
        @if ($template->description)
            <p class="mt-1 text-stone-600 text-sm">{{ $template->description }}</p>
        @endif
        <p class="mt-2 text-xs text-stone-500 font-mono">{{ $template->key }}</p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6"
        ...
        <div>
            ...
            <form method="POST" action="{{ route('admin.email-templates.update', $template) }}" class="bg-white rounded-2xl border border-stone-200 p-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input id="subject" name="subject" type="text" required maxlength="255"
                           x-model="subject"
                           class="mt-1 block w-full rounded-lg border-stone-300 font-mono text-sm"
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-stone-500">Markdown supported (<code>**bold**</code>, <code>[link](url)</code>). Separate paragraphs with a blank line — each paragraph becomes a line in the email.</p>
                    <textarea id="body" name="body" rows="14" required maxlength="5000"
                              x-model="body"
                              class="mt-2 block w-full rounded-lg border-stone-300 font-mono text-sm"
                ...
                <div class="flex items-center gap-3 pt-2 border-t border-stone-100">
                    ...
                    <button type="submit"
                            ...
                            class="inline-flex items-center px-4 py-2 rounded-full bg-stone-100 text-stone-800 hover:bg-stone-200 text-sm font-medium"
                            data-testid="preview-template">Preview ↗</button>
                    <a href="{{ route('admin.email-templates.index') }}" class="text-sm text-stone-500 hover:text-stone-900">Cancel</a>
                </div>
            </form>
        ...
        <aside class="bg-white rounded-2xl border border-stone-200 p-5 self-start sticky top-6">
            <h2 class="text-sm font-semibold text-stone-900 mb-2">Available placeholders</h2>
            @if (count($placeholders) === 0)
                <p class="text-xs text-stone-500">No placeholders documented for this template.</p>
            @else
                <p class="text-xs text-stone-500 mb-3">Wrap in curly braces, e.g. <code>{task_title}</code>. Unknown placeholders are left literal so typos are obvious in the rendered email.</p>
                <dl class="space-y-2.5" data-testid="placeholder-docs">
                    @foreach ($placeholders as $name => $description)
                        <div>
                            <dt><code class="text-xs bg-stone-100 px-1.5 py-0.5 rounded text-stone-800">&#123;{{ $name }}&#125;</code></dt>
                            <dd class="mt-1 text-xs text-stone-600 leading-snug">{{ $description }}</dd>
                        </div>
                    @endforeach
                </dl>
            @endif
