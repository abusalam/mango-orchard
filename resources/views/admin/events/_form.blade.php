...
<form method="POST" action="{{ $action }}" class="space-y-6">
    ...
    <div>
        <label for="description" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Description</label>
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        <div>
            <label for="start_at" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Starts at</label>
        ...
        <div>
            <label for="end_at" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Ends at <span class="text-stone-400 font-normal">(optional)</span></label>
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        ...
        <div>
            <label for="location_url" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Location URL <span class="text-stone-400 font-normal">(map / meeting link)</span></label>
            <input type="url" name="location_url" id="location_url" maxlength="500"
                   value="{{ old('location_url', $event->location_url) }}"
                   placeholder="https://maps.google.com/?q=…"
                   class="mt-1 block w-full rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 shadow-sm focus:border-orange-400 focus:ring-orange-400">
            @error('location_url') <p class="mt-1 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
        </div>
    </div>

    <div class="grid sm:grid-cols-2 gap-4">
        <div>
            <label for="host" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Host / organiser <span class="text-stone-400 font-normal">(optional)</span></label>
            <input type="text" name="host" id="host" maxlength="180"
                   value="{{ old('host', $event->host) }}"
                   placeholder="e.g. Konkan Krishi Vidyapeeth"
                   class="mt-1 block w-full rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 shadow-sm focus:border-orange-400 focus:ring-orange-400">
            @error('host') <p class="mt-1 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
        </div>
        <div>
            <label for="capacity" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Capacity <span class="text-stone-400 font-normal">(optional)</span></label>
            <input type="number" name="capacity" id="capacity" min="1" max="100000"
                   value="{{ old('capacity', $event->capacity) }}"
                   class="mt-1 block w-full rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 shadow-sm focus:border-orange-400 focus:ring-orange-400">
            @error('capacity') <p class="mt-1 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
        </div>
    </div>

    <div>
        <label for="registration_url" class="block text-sm font-medium text-stone-800 dark:text-stone-200">External registration URL <span class="text-stone-400 font-normal">(optional)</span></label>
        <input type="url" name="registration_url" id="registration_url" maxlength="500"
               ...
               class="mt-1 block w-full rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 shadow-sm focus:border-orange-400 focus:ring-orange-400">
        <p class="mt-1 text-xs text-stone-500 dark:text-stone-400">If you collect signups elsewhere, paste the link here. The event page will show a "Register" button pointing to it.</p>
        @error('registration_url') <p class="mt-1 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
    </div>

    <div>
        <label for="status" class="block text-sm font-medium text-stone-800 dark:text-stone-200">Status</label>
        
        <select name="status" id="status" required
                class="mt-1 block w-full rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 shadow-sm focus:border-orange-400 focus:ring-orange-400"
    >
            @foreach (\App\Modules\MangoOrchard\Models\Event::STATUSES as $value => $label)
                <option class="bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100" value="{{ $value }}" @selected(old('status', $event->status) === $value)>{{ $label }}</option>
            @endforeach
        </select>
    ...
    <div class="flex items-center gap-3 pt-4 border-t border-stone-100 dark:border-stone-800">
        ...
        <a href="{{ route('admin.events.index') }}" class="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-900 dark:text-stone-100">Cancel</a>
    </div>
</form>
