<x-admin-layout title="Role applications" active="role-applications">
    <header class="mb-6">
        ...
        <p class="mt-1 text-stone-600 dark:text-stone-300">Users requesting elevated roles. Approving assigns the role immediately.</p>
    </header>

    <section class="mb-10">
        <h2 class="text-lg font-medium text-stone-900 dark:text-stone-100 mb-3">Pending <span class="text-stone-400 text-sm">({{ $pending->count() }})</span></h2>

        @if ($pending->isEmpty())
            <div class="rounded-2xl border border-dashed border-stone-300 p-10 text-center text-stone-500 dark:text-stone-400" data-testid="role-applications-empty">
                Nothing waiting for review.
            </div>
        @else
            <div class="space-y-4">
                @foreach ($pending as $application)
                    <article class="bg-white dark:bg-stone-950 rounded-2xl border border-stone-200 dark:border-stone-800 p-5 sm:p-6" data-testid="pending-application">
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                            <div>
                                <p class="text-base font-semibold text-stone-900 dark:text-stone-100">{{ $application->user->name }}
                                    <span class="text-xs font-normal text-stone-500 dark:text-stone-400">{{ $application->user->email }}</span>
                                </p>
                                <p class="mt-1 text-sm text-stone-700 dark:text-stone-300">
                                    Requesting role:
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-800 dark:text-stone-200 text-xs font-medium border border-stone-200 dark:border-stone-800">{{ $application->role->name }}</span>
                                </p>
                                @if ($application->message)
                                    <blockquote class="mt-3 text-sm text-stone-600 dark:text-stone-300 border-l-2 border-stone-200 dark:border-stone-800 pl-3 italic">{{ $application->message }}</blockquote>
                                @endif
                            ...
                            <div class="flex gap-2 shrink-0">
                                ...
                                >
                                    <button type="button" class="inline-flex items-center px-4 py-2 rounded-full border border-rose-300 text-rose-700 dark:text-rose-400 font-medium hover:bg-rose-50 transition-colors text-xs">Reject</button>
                                </x-confirm-form>
                            </div>
                        </div>
                    </article>
    ...
    <section>
        <h2 class="text-lg font-medium text-stone-900 dark:text-stone-100 mb-3">Recently reviewed</h2>

        @if ($reviewed->isEmpty())
            <p class="text-sm text-stone-500 dark:text-stone-400">No applications have been reviewed yet.</p>
        @else
