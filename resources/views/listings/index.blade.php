<x-site-layout :title="'Marketplace — '.config('app.name')">
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <header class="mb-10">
            ...
            <p class="mt-3 text-stone-600 dark:text-stone-300 max-w-2xl">Listings posted by orchard owners. Reach out to them directly using the contact details on each listing.</p>
            @guest
                <div class="mt-5">
            ...
            @else
                ...
                @else
                    <p class="mt-5 text-sm text-stone-500 dark:text-stone-400">Want to list your own mangoes? Ask an admin to grant you the <code class="px-1.5 py-0.5 bg-stone-100 dark:bg-stone-800 rounded text-stone-700 dark:text-stone-300">grower</code> role.</p>
                @endcan
            @endguest
        </header>

        <form method="GET" action="{{ route('listings.index') }}" class="mb-6 flex flex-wrap items-center gap-3">
            <label for="variety" class="text-sm text-stone-700 dark:text-stone-300">Filter by variety:</label>
            
        <select name="variety" id="variety" onchange="this.form.submit()"
                    class="rounded-lg border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 text-sm focus:border-orange-400 focus:ring-orange-400"
    >
                <option class="bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100" value="">All varieties</option>
                @foreach ($varieties as $variety)
                    <option class="bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100" value="{{ $variety->id }}" @selected($filterVarietyId === $variety->id)>{{ $variety->name }}</option>
                @endforeach
            </select>
            @if ($filterVarietyId)
                <a href="{{ route('listings.index') }}" class="text-xs text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:text-stone-100">Clear</a>
            @endif
            <p class="ml-auto text-sm text-stone-500 dark:text-stone-400">{{ $listings->total() }} {{ Str::plural('listing', $listings->total()) }}</p>
        </form>

        @if ($listings->isEmpty())
            <div class="rounded-2xl border border-dashed border-stone-300 p-12 text-center">
                <p class="text-stone-600 dark:text-stone-300">No listings here yet.</p>
                @can(\App\Permissions::LISTINGS_MANAGE)
                    ...
                    <article class="group relative overflow-hidden rounded-2xl bg-white dark:bg-stone-950 border border-stone-200/80 hover:border-stone-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                        <a href="{{ route('listings.show', $listing) }}" class="block">
                            ...
                            <div class="p-5">
                                ...
                                <p class="mt-1 text-sm text-stone-500 dark:text-stone-400">{{ $listing->variety->name }} · {{ $listing->location }}</p>
                                @if ($listing->price_per_kg)
                                    <p class="mt-3 text-sm font-medium text-stone-800 dark:text-stone-200">₹{{ number_format((float) $listing->price_per_kg, 2) }} <span class="text-stone-500 dark:text-stone-400 font-normal">/ kg</span></p>
                                @endif
                                @if ($listing->quantity_available_kg)
                                    <p class="text-xs text-stone-500 dark:text-stone-400">~{{ number_format($listing->quantity_available_kg) }} kg available</p>
                                @endif
                            </div>
                        </a>
                    </article>
