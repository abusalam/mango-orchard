<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-stone-900 leading-tight">{{ __('Dashboard') }}</h2>
                <p class="text-sm text-stone-500">Welcome back, {{ $user->name }}.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            @if (! empty($attention))
                <section data-testid="dashboard-attention" class="space-y-3">
                    <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500">Needs your attention</h3>

                    @isset($attention['pending_role_applications'])
                        <a href="{{ $attention['pending_role_applications']['href'] }}"
                           class="block bg-amber-50 border border-amber-200 rounded-2xl p-4 hover:border-amber-400 transition-colors"
                    ...
                    @isset($attention['my_application_decision'])
                        ...
                        <a href="{{ $attention['my_application_decision']['href'] }}"
                           class="block rounded-2xl p-4 transition-colors {{ $decision->status === \App\Models\RoleApplication::STATUS_APPROVED
                            ...
                            <p class="text-sm {{ $decision->status === \App\Models\RoleApplication::STATUS_APPROVED ? 'text-emerald-900' : 'text-stone-700' }}">
            ...
            <section data-testid="dashboard-personal" class="space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white border border-stone-200 rounded-2xl p-5">
                        <p class="text-xs uppercase tracking-wide text-stone-500">Your role</p>
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            @forelse ($personal['roles'] as $roleName)
                                <span @class([
                                    'inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border',
                                    'bg-amber-100 text-amber-900 border-amber-200' => $roleName === \App\Roles::SUPERUSER,
                                    'bg-stone-100 text-stone-700 border-stone-200' => $roleName !== \App\Roles::SUPERUSER,
                                ])>{{ $roleName }}</span>
                            @empty
                    ...
                    @isset($personal['listings'])
                        <a href="{{ route('my.listings.index') }}"
                           class="block bg-white border border-stone-200 rounded-2xl p-5 hover:border-stone-400 transition-colors">
                            <p class="text-xs uppercase tracking-wide text-stone-500">Your listings</p>
                            <p class="mt-2 text-3xl font-semibold text-stone-900">{{ $personal['listings']['total'] }}</p>
                            <div class="mt-3 flex flex-wrap gap-2 text-[11px]">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-stone-100 text-stone-700 border border-stone-200">{{ $personal['listings']['draft'] }} draft</span>
                    ...
                    @if (count($personal['roles']) === 0 && ! $personal['pending_application'] && ! isset($personal['listings']))
                        <a href="{{ route('profile.edit') }}"
                           class="block bg-amber-50 border border-amber-200 rounded-2xl p-5 hover:border-amber-400 transition-colors">
            ...
            @if ($orchard)
                <section data-testid="dashboard-orchard" class="space-y-3">
                    <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500">At a glance</h3>

                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">

                        @isset($orchard['users'])
                            <div class="bg-white border border-stone-200 rounded-2xl p-5">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Users</p>
                                <p class="mt-2 text-3xl font-semibold text-stone-900">{{ number_format($orchard['users']['total']) }}</p>
                                <p class="mt-1 text-[11px] text-stone-500">+{{ $orchard['users']['last_week'] }} this week</p>
                            </div>
                        @endisset

                        @isset($orchard['listings'])
                            <div class="bg-white border border-stone-200 rounded-2xl p-5">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Listings live</p>
                                <p class="mt-2 text-3xl font-semibold text-stone-900">{{ number_format($orchard['listings']['published']) }}</p>
                                <p class="mt-1 text-[11px] text-stone-500">+{{ $orchard['listings']['last_week'] }} this week</p>
                            </div>
                        @endisset

                        @isset($orchard['varieties'])
                            <div class="bg-white border border-stone-200 rounded-2xl p-5">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Varieties</p>
                                <p class="mt-2 text-3xl font-semibold text-stone-900">{{ number_format($orchard['varieties']['total']) }}</p>
                                <p class="mt-1 text-[11px] text-stone-500">in the catalogue</p>
                            </div>
                        @endisset

                        @isset($orchard['events'])
                            <div class="bg-white border border-stone-200 rounded-2xl p-5">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Upcoming events</p>
                                <p class="mt-2 text-3xl font-semibold text-stone-900" data-testid="orchard-events-upcoming">{{ number_format($orchard['events']['upcoming']) }}</p>
                                <p class="mt-1 text-[11px] text-stone-500">scheduled ahead</p>
                            </div>
                        @endisset

                        @isset($orchard['telemetry_24h'])
                            <div class="bg-white border border-stone-200 rounded-2xl p-5">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Events / 24h</p>
                                <p class="mt-2 text-3xl font-semibold text-stone-900">{{ number_format($orchard['telemetry_24h']) }}</p>
                                <p class="mt-1 text-[11px] text-stone-500">app-wide telemetry</p>
                            </div>
                        @endisset
                    </div>
                </section>
            ...
            @if ($telemetryInsights !== null)
                <section data-testid="dashboard-telemetry-insights" class="space-y-3">
                    <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500">Telemetry insights</h3>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="bg-white border border-stone-200 rounded-2xl p-5">
                            <div class="flex items-baseline justify-between">
                                <p class="text-xs uppercase tracking-wide text-stone-500">Events / day · last 14 days</p>
                        ...
                        <div class="bg-white border border-stone-200 rounded-2xl p-5" data-testid="telemetry-auth-health">
                            <p class="text-xs uppercase tracking-wide text-stone-500">Auth health · last 7 days</p>
                            ...
                            <div class="mt-3 flex items-baseline gap-4">
                                <div>
                                    ...
                                    <p class="text-[11px] text-stone-500">login success rate</p>
                                </div>
                                <div class="text-right ml-auto space-y-0.5">
                                    <p class="text-xs text-stone-700"><strong class="text-emerald-700">{{ number_format($health['succeeded']) }}</strong> succeeded</p>
                                    <p class="text-xs text-stone-700"><strong class="text-rose-700">{{ number_format($health['failed']) }}</strong> failed</p>
                                    @if ($health['captcha_failed'] > 0)
                                        <p class="text-xs text-stone-700"><strong class="text-amber-700">{{ number_format($health['captcha_failed']) }}</strong> captcha failed</p>
                                    @endif
                                </div>
                            </div>

                            @if ($totalAuth > 0)
                                <div class="mt-4 h-2 rounded-full overflow-hidden bg-stone-100 flex" aria-hidden="true">
                            ...
                            <p class="mt-4 text-xs text-stone-500"><strong data-testid="telemetry-active-users-7d">{{ number_format($telemetryInsights['active_users_7d']) }}</strong> distinct users active in the last 7 days</p>
                        </div>

                        {{-- Top events bar chart (7d) --}}
                        <div class="bg-white border border-stone-200 rounded-2xl p-5 lg:col-span-2">
                            <p class="text-xs uppercase tracking-wide text-stone-500">Top events · last 7 days</p>
                                ...
                                <div class="mt-4 space-y-2" data-testid="telemetry-top-events">
                                    @foreach ($telemetryInsights['top_events'] as $row)
                                        ...
                                        <div class="flex items-center gap-3 text-xs">
                                            <a href="{{ route('admin.telemetry.index', ['event' => $row['event']]) }}" class="font-mono text-stone-700 w-44 sm:w-56 truncate hover:text-stone-900" title="{{ $row['event'] }}">{{ $row['event'] }}</a>
                                            ...
                                            <span class="font-medium text-stone-700 w-10 text-right" data-testid="telemetry-top-events-count">{{ number_format($row['count']) }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
            ...
            @if ($recentActivity !== null)
                <section data-testid="dashboard-activity">
                    <div class="flex items-end justify-between mb-3">
                        <h3 class="text-xs font-semibold tracking-wide uppercase text-stone-500">Latest activity</h3>
                    ...
                    <div class="bg-white border border-stone-200 rounded-2xl overflow-hidden">
                        @if ($recentActivity->isEmpty())
                            <p class="p-6 text-sm text-stone-500 italic">No activity yet.</p>
                        @else
                            <ul class="divide-y divide-stone-100 text-sm">
                                @foreach ($recentActivity as $event)
                                    <li class="px-5 py-3 flex items-center gap-3" data-testid="dashboard-activity-row">
                                        <span class="font-mono text-xs text-stone-700">{{ $event->event }}</span>
