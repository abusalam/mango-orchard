<x-site-layout :title="$event->title.' — Training & Events'">
    <section class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <nav class="text-sm text-stone-500 mb-6">
            ...
            <span class="text-stone-800">{{ $event->title }}</span>
        </nav>
        ...
        <div class="rounded-3xl overflow-hidden border border-stone-200 bg-white shadow-sm">
            ...
            <div class="p-8 sm:p-10">
                ...
                <p class="mt-2 text-stone-500">{{ $isOnline ? 'Online event' : $event->location }}</p>

                <dl class="mt-6 grid sm:grid-cols-2 gap-4 text-sm border-y border-stone-100 py-6">
                    <div>
                    ...
                    <div>
                        <dt class="text-stone-500">Where</dt>
                        <dd class="font-medium text-stone-800">
                    ...
                    @if ($event->host)
                        <div>
                            <dt class="text-stone-500">Hosted by</dt>
                            <dd class="font-medium text-stone-800">{{ $event->host }}</dd>
                        </div>
                    @endif
                    @if ($event->capacity)
                        <div>
                            <dt class="text-stone-500">Capacity</dt>
                            <dd class="font-medium text-stone-800">{{ number_format($event->capacity) }} attendees</dd>
                        </div>
                    @endif
                </dl>
                ...
                @can(\App\Permissions::EVENTS_MANAGE)
                    <div class="mt-8 pt-6 border-t border-stone-100 flex gap-3">
                        ...
                        <a href="{{ route('admin.events.index') }}" class="text-sm text-stone-600 hover:text-stone-900 self-center">All events →</a>
                    </div>
                @endcan
            </div>
        </div>
